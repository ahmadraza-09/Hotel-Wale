...
const HotelHero = () => {
  ...
  const destinationsList = cities.map((city) => city.name);
  ...
  const handleSubmit = (e) => {
    ...
    if (destination) {
      ...
      navigate(
        `/hotels/hotels-in-${destination.replace(/\s+/g, "-").toLowerCase()}`
      );
    }
  };
      ...
      <div className="max-w-9xl w-full mx-auto bg-gray-100 rounded-2xl px-5 py-5">
        ...
        <div className="flex justify-around sm:justify-start sm:gap-10 mb-6 py-5">
          ...
          <div className="flex flex-col gap-2 justify-center items-center">
            <button
              className={`w-[52px] h-[52px] py-2 px-4 rounded-md shadow-custom ${
                isActive("/") || isActive("/hotel")
                  ? "bg-myColor text-white"
                  : "bg-white text-gray-800"
              } hover:bg-myColor hover:text-white`}
              onClick={() => {
                navigate("/hotels");
              }}
            >
            ...
            <span className="font-TTHovesRegular text-sm text-center">
              Hotels
            </span>
          </div>

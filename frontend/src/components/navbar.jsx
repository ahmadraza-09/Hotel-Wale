...
import UserDark from "../assets/icons/user-icon-dark.svg";
import MenuBar from "../assets/icons/menu-bar-icon.svg";
import MenuBarDark from "../assets/icons/menu-bar-icon-dark.svg";
...
const Navbar = () => {
  ...
  // Listen for theme changes in localStorage (from sidebar or anywhere)
  useEffect(() => {
    const syncTheme = () => {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-white dark:bg-gray-900 shadow-sm flex justify-between px-4 py-4 sm:px-5 sm:py-4 text-black dark:text-white transition-colors">
        <div className="flex gap-5 justify-center items-center text-center">
          <img
            src={theme === "dark" ? MenuBarDark : MenuBar}
            alt="Menu"
        ...
        {isLoggedIn ? (
          <div className="flex gap-5 justify-center items-center text-center">
            ...
            <button
              className="flex gap-2 justify-center items-center text-center border-[0.5px] px-2 py-[5px] border-black dark:border-white rounded-lg"
              onClick={() => navigate("/profile")}
            >
              {!profile_image || profile_image === "null" ? (
                <img src={theme === "dark" ? UserDark : User} alt="User" />
              ) : (
                <img
                  src={profile_image}
                  alt="Profile"
